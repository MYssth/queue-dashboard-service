...
async function getHNStatus(){
    try{
        ...
        const tempDate = dateFns.format(dateFns.addYears(new Date(), 543), 'yyyyMMdd');

        const giveMed = await pool.request().input('DATE', sql.VarChar, tempDate)
        ...
        const payBill = await pool.request().input('DATE', sql.VarChar, tempDate)
        ...
        const treatEnd = await pool.request().input('DATE', sql.VarChar, tempDate)
        ...
        const treatProc = await pool.request().input('DATE', sql.VarChar, tempDate)
        ...
        const treatWait = await pool.request().input('DATE', sql.VarChar, tempDate)
        ...
        const register = await pool.request().input('DATE', sql.VarChar, tempDate)
